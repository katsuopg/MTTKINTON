...
import type { DomMechItem, DomItemStatus, DomItemCategory, DomMasters, DOM_ITEM_STATUSES } from '@/types/dom';
...
const CATEGORY_OPTIONS: { value: DomItemCategory; labels: Record<Language, string> }[] = [
  { value: 'make', labels: { ja: 'M', en: 'M', th: 'M' } },
  { value: 'buy', labels: { ja: 'P', en: 'P', th: 'P' } },
];
  ...
  return (
    <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
      ...
      <td className="px-1 py-1 text-center text-sm text-gray-500 whitespace-nowrap">
        {readOnly ? (
          <span>{item.category === 'buy' ? `P${item.item_number ?? ''}` : (item.item_number ?? '')}</span>
        ) : (
          <div className="flex items-center justify-center gap-0">
            {item.category === 'buy' && <span className="text-gray-400">P</span>}
            <input
              type="number"
              value={item.item_number ?? ''}
              onChange={(e) => onChange('item_number', e.target.value === '' ? null : Number(e.target.value))}
              className="w-8 text-center text-sm border-0 bg-transparent focus:ring-1 focus:ring-brand-500 dark:text-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            />
          </div>
        )}
      </td>
      ...
      {/* 図番/型式（製作品→part_code、購入品→model_number） */}
      <td className="px-1 py-1">
        <EditableCell
          value={item.category === 'buy' ? (item.model_number ?? null) : (item.part_code ?? null)}
          onChange={(v) => onChange(item.category === 'buy' ? 'model_number' : 'part_code', v)}
          placeholder={item.category === 'buy'
            ? (language === 'ja' ? '型式' : 'Model')
            : (language === 'ja' ? '図番' : 'Dwg No.')}
          readOnly={readOnly}
        />
      </td>
      ...
      {/* 金額 */}
      <td className="px-1 py-1 text-right text-sm text-gray-700 dark:text-gray-300">
