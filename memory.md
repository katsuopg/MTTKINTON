...
### テーブル幅の修正（max-w-7xl削除）
- **問題**: テーブルの幅が画面の最後まで広がらない
- **原因**: `max-w-7xl`クラスがテーブルコンテナの幅を制限していた
- **修正内容**:
  1. `CostManagementContent.tsx` - `max-w-7xl overflow-x-auto` → `overflow-x-auto`
  2. `project-management/page.tsx` - 同様の修正
- **結果**: テーブルが画面幅いっぱいに表示されるようになった
