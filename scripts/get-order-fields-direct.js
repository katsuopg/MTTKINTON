...
const KINTONE_DOMAIN = envVars.KINTONE_DOMAIN;
const API_TOKEN = envVars.KINTONE_API_TOKEN_ORDER;
const APP_ID = envVars.KINTONE_APP_ORDER_MANAGEMENT;
...
async function getOrderFields() {
  ...
  try {
    ...
    Object.entries(data.properties).forEach(([fieldCode, fieldInfo]) => {
      if (fieldInfo.type === 'SUBTABLE') {
        console.log(`\n[サブテーブル] ${fieldCode}:`);
        if (fieldInfo.fields) {
          Object.entries(fieldInfo.fields).forEach(([subFieldCode, subFieldInfo]) => {
