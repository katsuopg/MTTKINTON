      ...
      for (const customer of batch) {
        try {
          const data = {
            ...
            short_name: customer.Cs_Name?.value || null,
            ...
            tax_id: customer.文字列__1行__11?.value || customer.文字列__1行__0?.value || null,
            payment_terms: customer.ドロップダウン?.value || null,
            address: customer.複数行__0?.value || customer.住所?.value || null,
            postal_code: customer.郵便番号?.value || null,
            website_url: customer.リンク_ウェブサイト?.value || null,
            notes: customer.文字列__複数行_?.value || customer.備考?.value || null,
